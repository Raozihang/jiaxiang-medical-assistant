...
func (h *ImportHandler) ImportVisits(c *gin.Context) {
	...
	result, err := h.importService.SubmitVisits(c.Request.Context(), req)
	if err != nil {
...
func (h *ImportHandler) TaskDetail(c *gin.Context) {
	result, err := h.importService.GetTask(c.Request.Context(), c.Param("id"))
	if err != nil {
		handleDomainError(c, err)
		return
	}
...
func (h *ImportHandler) TaskList(c *gin.Context) {
	...
	response.Success(c, gin.H{"items": result.Items, "page": result.Page, "page_size": result.PageSize, "total": result.Total})
}
